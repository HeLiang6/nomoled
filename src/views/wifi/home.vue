<template>
  <div class="main-box">
    ...
    <div class="baidu_map">
      ...
      <baidu-map
        :center="$store.state.wifi.map_center"
        :zoom="zoom"
        @ready="handler"
        class="bm-view"
        @click="getClickInfo"
      >
        <template v-for="(item,index) in $store.state.wifi.wifiMap">
          <!-- dragging 代表图标是否可以拖动 -->
          <BmMarker
            :key="index"
            :position="item.site"
...
export default {
  ...
  methods: {
    ...
    // 地图创建
    handler ({ BMap, map }) {
      console.log(this.$store.state.wifi.map_center)
      // this.zoom = 14
      map.setMapStyleV2({
